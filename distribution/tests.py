...
class SettlementWorkflowTests(TestCase):
    ...
    def test_admin_uses_application_sidebar_and_settings_links(self):
        ...
        self.assertContains(response, 'class="admin-hub"')
        self.assertContains(response, 'data-admin-panel="distribution"')
        self.assertContains(response, 'data-admin-panel="auditlog"')
        self.assertContains(response, 'data-admin-panel="axes"')
        self.assertNotContains(response, '<h1>Panel administracyjny</h1>')
        admin_html = response.content.decode()
        self.assertLess(admin_html.index('data-admin-tab="distribution"'), admin_html.index('data-admin-tab="auditlog"'))
        self.assertLess(admin_html.index('data-admin-tab="auditlog"'), admin_html.index('data-admin-tab="axes"'))
